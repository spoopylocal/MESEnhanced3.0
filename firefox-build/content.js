...
function showUpdatePopup(status) {
  ...
  title.className = "mes-update-title";
  title.textContent = `Update Available: v${status.latestVersion}`;

  const sub = document.createElement("div");
  sub.className = "mes-update-sub";
  sub.textContent = `Current version: v${status.currentVersion || "unknown"}`;

  const notesWrap = document.createElement("div");
  notesWrap.className = "mes-update-notes";
  const notesList = document.createElement("ul");
  const notes = Array.isArray(status.notes) ? status.notes.slice(0, 5) : [];
  const noteValues = notes.length ? notes : ["Performance improvements and fixes"];
  ...
  actions.className = "mes-update-actions";
  const updateBtn = document.createElement("button");
  updateBtn.type = "button";
  updateBtn.className = "mes-update-btn";
  updateBtn.id = "mes-update-now";
  updateBtn.textContent = "Update";

  const laterBtn = document.createElement("button");
  laterBtn.type = "button";
  laterBtn.className = "mes-update-btn secondary";
  laterBtn.id = "mes-update-later";
...
const renderLpnContents = (contents, isLoading = false) => {
  ...
  if (isLoading) {
    wrapper.className = 'mes-lpn-no-contents';
    wrapper.textContent = 'Loading contents...';
    return wrapper;
  }
  
  if (!contents || contents.length === 0) {
    wrapper.className = 'mes-lpn-no-contents';
    wrapper.textContent = 'No contents found';
    return wrapper;
  }

  wrapper.className = 'mes-lpn-contents';
  const table = document.createElement('table');
  table.className = 'mes-lpn-contents-table';
  const thead = document.createElement('thead');
  const headerRow = document.createElement('tr');
  ['Item', 'Qty', 'Inner LPN', 'Locator'].forEach((label) => {
  ...
  contents.forEach(item => {
    ...
    itemTd.textContent = String(itemName);
    const qtyTd = document.createElement('td');
    qtyTd.textContent = String(qty);
    const lpnTd = document.createElement('td');
    lpnTd.className = 'mes-lpn-mini';
    lpnTd.textContent = String(lpn);
    const locTd = document.createElement('td');
    locTd.textContent = String(locator);

    tr.appendChild(itemTd);
    tr.appendChild(qtyTd);
    tr.appendChild(lpnTd);
...
const setExpandedRowContent = (expandedRow, contents, isLoading = false) => {
  const td = document.createElement('td');
  td.colSpan = 100;
  td.appendChild(renderLpnContents(contents, isLoading));
  expandedRow.replaceChildren(td);
};
...
const toggleLpnDropdown = async (button, lpn) => {
  ...
  if (isExpanded) {
    ...
    if (lpnContentsCache.hasOwnProperty(lpn)) {
      // Use cached data
      setExpandedRowContent(expandedRow, lpnContentsCache[lpn]);
      row.parentNode.insertBefore(expandedRow, row.nextSibling);
    } else {
      // Not in cache (failed on page load), fetch now
      setExpandedRowContent(expandedRow, null, true);
      row.parentNode.insertBefore(expandedRow, row.nextSibling);
      ...
      if (contents !== null) {
        lpnContentsCache[lpn] = contents;
        setExpandedRowContent(expandedRow, contents);
      } else {
        // Still failed, show empty
        setExpandedRowContent(expandedRow, []);
      }
    }
  }
};
...
const openSerialScanModal = ({ total, affected, lines }) => {
  ...
  const countBadge = (txt, cls = "") => {
    const badge = document.createElement("span");
    badge.className = `ssc-badge ${cls}`.trim();
  ...
  const rolePill = (role) => {
    ...
    pill.className = `ssc-pill ${role === "NODE" ? "node" : ""}`.trim();
    pill.textContent = role === "MAIN" ? "Main Device" : "Node";
    return pill;
  };
  ...
  title.textContent = "Missing Attributes";
  const actions = document.createElement("div");
  actions.className = "ssc-actions";
  const copyBtn = document.createElement("button");
  copyBtn.className = "ssc-btn";
  copyBtn.id = "sscCopy";
  copyBtn.textContent = "Copy";
  const closeBtn = document.createElement("button");
  closeBtn.className = "ssc-btn primary";
  closeBtn.id = "sscClose";
  ...
  badges.className = "ssc-badges";
  badges.appendChild(countBadge(`${total} total`));
  badges.appendChild(countBadge(`${affected.length} with missing`, "warn"));
  ...
  if (affected.length) {
    affected.forEach((a) => {
      ...
      item.className = "ssc-item";
      const row = document.createElement("div");
      row.className = "ssc-row";
      const serial = document.createElement("strong");
      serial.className = "ssc-serial";
      serial.textContent = a.serial;
      const missBadge = document.createElement("span");
      missBadge.className = "ssc-badge warn";
      missBadge.textContent = `${a.miss.length} missing`;
      row.appendChild(serial);
      row.appendChild(rolePill(a.role));
      row.appendChild(missBadge);

      const missing = document.createElement("div");
      missing.className = "ssc-missing";
      const missingLabel = document.createElement("strong");
      missingLabel.textContent = "Missing:";
      missing.appendChild(missingLabel);
      missing.appendChild(document.createTextNode(` ${a.miss.join(", ")}`));
    ...
    const item = document.createElement("div");
    item.className = "ssc-item";
    item.textContent = "All serials complete (MAC fields ignored).";
    list.appendChild(item);
  }

  const pre = document.createElement("div");
  pre.className = "ssc-pre";
  pre.id = "sscPre";
...
function interceptGeneratePlacardClick() {
  // Firefox build intentionally disables placard interception/auto-fill.
}
...
// interceptGeneratePlacardClick(); // Disabled for Firefox build
